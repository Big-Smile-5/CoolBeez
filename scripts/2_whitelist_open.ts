...
async function main() {
  ...
  let leafNodes = CollectionConfig.whitelistAddresses.map(addr => keccak256(addr));
  ...
  leafNodes = CollectionConfig.freelistAddresses.map(addr => keccak256(addr));
  merkleTree = new MerkleTree(leafNodes, keccak256, { sortPairs: true });
  rootHash = '0x' + merkleTree.getRoot().toString('hex');

  if ((await contract.merkleRootForFreeAddress()) !== rootHash) {
    console.log(`Updating the root hash of freeaddress to: ${rootHash}`);

    await (await contract.setMerkleRootForFreeAddress(rootHash)).wait();
  }
